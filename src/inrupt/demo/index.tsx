...
export default function InruptDemo() {
    ...
    useEffect(() => {
        (async () => {
            const dataset = await getSolidDataset(session.info.webId!, {fetch: session.fetch});
            setDataset(dataset);
            const profile = getThing(dataset, session.info.webId!)!;
            const name = getLiteral(profile, FOAF.name);
            setValue("name", name?.value || "")
        })();
    }, [session.fetch, session.info.webId, setValue]);
    ...
    const onSubmit: SubmitHandler<FormData> = async (data) => {
        ...
        const savedDataset = await saveSolidDatasetAt(session.info.webId!, updatedDataset, {fetch: session.fetch});
