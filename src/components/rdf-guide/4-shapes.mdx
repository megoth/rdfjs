...
<Content>
    ...
    Shapes are standards that describe which attributes a RDF graph should have. They can be used to validate RDF
    graphs (e.g. graphs with values that we've created using user input), but more importantly they allow a human
    ...
    but the trust is probably something like "it depends".
