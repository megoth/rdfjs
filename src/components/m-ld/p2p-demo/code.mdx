...
<Code language="tsx" id="P2PDemoCode" code={p2pDemoCode} className="line-numbers">
    ...
    <CodeStep lines={["51-54"]} title="Starting peers">
        Once we've set up the genesis clone, we load the peers by requesting the URL `/m-ld/[domainId]` in two separate
        iframes. See the code for the next part to learn how they work.
    </CodeStep>
    <CodeStep lines={["47"]} title="Loading state">
        While we wait for everything to get into place, we show the `Loading` component.
    </CodeStep>
    ...
    <CodeStep lines={["17", "42", "49-50"]} title="Error handling">
