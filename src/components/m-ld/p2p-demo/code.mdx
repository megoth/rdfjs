...
<Code language="tsx" id="P2PDemoCode" code={p2pDemoCode} className="line-numbers">
    <CodeStep lines={["15-16"]} title={<>Setting up <code>domainId</code> and <code>domainUrl</code></>}>
    ...
    <CodeStep lines={["17", "23", "48"]} title="Waiting until user initiates the demo">
    ...
    <CodeStep lines={["18", "44", "50"]} title="Waiting until genesis clone is finished setting up">
        I want to make sure that the initial clone is properly set up before loading in the peers.
    </CodeStep>
    <CodeStep lines={["24-43"]} title="Starting genesis clone">
