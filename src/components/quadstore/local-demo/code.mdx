...
<Content>
    This demo loads and saves a given name to `localStorage`. If no name is retrieved from `localStorage`, it will
    use a pre-set name "Test".
</Content>
...
<Code language="tsx" id="QuadstoreLocalDemo" code={demoCode} className="line-numbers">
    ...
    <CodeStep lines={["16", "21-25"]} title="Getting name from localStorage">
    ...
    <CodeStep lines={["33-40"]} title={<>Update RDF graph and <code>localStorage</code></>}>
